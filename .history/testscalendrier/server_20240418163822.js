...
MongoClient.connect(url, { useNewUrlParser: true, useUnifiedTopology: true }, (err, client) => {
    ...
    // Route pour vérifier si un créneau est déjà réservé
    app.post('/checkReservation', (req, res) => {
        const { day, hour } = req.body;

        collection.findOne({ [hour]: { [day]: true } }, (err, result) => {
            if (err) {
                console.error(err);
                return res.status(500).json({ error: 'Erreur lors de la vérification de la réservation' });
            }
            
            // Si le créneau est trouvé, il est réservé ; sinon, il est disponible
            const isReserved = result !== null;
            res.json({ reserved: isReserved });
        });
    });

    // Ajoutez une nouvelle route pour récupérer les réservations existantes
